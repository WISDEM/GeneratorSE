...
class geom_error(Exception):
    """Exception raised for errors in boundary conditions"""
    pass
...
class FEMM_Geometry(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_discrete_input("k_sfil", 0.65, desc="number of phases")
        ...
        self.add_discrete_input("alpha_m", 0.5, desc="pole pair width")
        ...
        self.add_output("B_smax", 0.0, desc="Peak flux density in the stator")
    ...
    def compute(self, inputs, outputs, discrete_inputs, discrete_outputs):
        ...
        alpha_m = float(discrete_inputs["alpha_m"])
        ...
        outputs["tau_s"]      =theta_p_r*5/6*D_a/2
        
        tau_s=float(outputs["tau_s"])
        
        outputs["tau_p"]=tau_p     =np.pi*r_g/p1 
         
        bs_taus    =0.5
         
        outputs["b_s"]       =0.5*tau_s
        
        b_s=float(outputs["b_s"])
...
# the depth dimension, and an angle constraint of 30 degrees
        ...
        h=[0.000000,82000.000000,232000.000000,392000.000000,557000.000000,717000.000000,872000.000000,1037000.000000,1352000.000000,1512000.000000]
...
#           femm.mi_addbhpoint('N48SH',b[i],h[i])
        ...
        xmag1=5*np.cos(np.deg2rad(theta_p_d))
        ymag1=5*np.sin(np.deg2rad(theta_p_d))
        
        xmag2=5*np.cos(np.deg2rad(theta_p_d/2))
        ymag2=5*np.sin(np.deg2rad(theta_p_d/2))
        ...
        X_2,Y_2=(r_g+d_mag+bridge_thickness)*np.cos(theta_1),(r_g+d_mag+bridge_thickness)*np.sin(theta_1)
        xc,yc=(r_g+d_mag+bridge_thickness)*np.cos(theta_p_r*0.5),(r_g+d_mag+bridge_thickness)*np.sin(theta_p_r*0.5)
        m1=np.tan(np.deg2rad(180-(180-alpha_v*0.5-np.rad2deg(theta_1))))

        X_3=(m1*X_1-Y_1)/m1
        
                
        X_5,Y_5=(r_g+d_mag+bridge_thickness)*np.cos(theta_m_r-theta_m_r2),(r_g+d_mag+bridge_thickness)*np.sin(theta_m_r-theta_m_r2)
        ...
        if ratio ==1:
            ...
            femm.mi_setgroup(1)
            air_1 = (Polygon(Point(xend, yend,evaluate=False),Point(r3[0].x,r3[0].y,evaluate=False),Point(X_8,Y_8,evaluate=False)))
            ...
            femm.mi_setgroup(1)
            air_1 = Polygon(Point(xend, yend,evaluate=False),Point(r3[0].x,r3[0].y,evaluate=False),Point(X_6,Y_6,evaluate=False),Point(X_8,Y_8,evaluate=False))
            mag = (Polygon(Point(xstart,ystart,evaluate=False), Point(X_8, Y_8,evaluate=False)))
            l1=mag.midpoint.evalf()
            

        air_2 = Polygon(Point(X_1, Y_1,evaluate=False),Point(X_2,Y_2,evaluate=False),Point(X_7,Y_7,evaluate=False),Point(xstart,ystart,evaluate=False))
...
#        
        ...
        femm.mi_selectlabel(r_mag_label*np.cos(np.deg2rad(theta_p_d+theta_p_d/4)+theta_m_r/2),r_mag_label*np.sin(np.deg2rad(theta_p_d+theta_p_d/4)+theta_m_r/2))
        femm.mi_setblockprop("N48SH", 1, 1, 0,mag_dir+theta_p_d+180,1,0 )
        femm.mi_clearselected()
        femm.mi_selectlabel(r_mag_label*np.cos(np.deg2rad(theta_p_d+theta_p_d*3/4)),r_mag_label*np.sin(np.deg2rad(theta_p_d+theta_p_d*3/4)))
        femm.mi_deleteselected()
        femm.mi_selectlabel(r_mag_label*np.cos(np.deg2rad(theta_p_d+theta_p_d/4)),r_mag_label*np.sin(np.deg2rad(theta_p_d+theta_p_d/4)))
        femm.mi_mirror(x0,y0,(r_g+d_mag+bridge_thickness)*np.cos(np.deg2rad(theta_p_d+theta_p_d/2)),(r_g+d_mag+bridge_thickness)*np.sin(np.deg2rad(theta_p_d+theta_p_d/2)))
        femm.mi_clearselected()
        ...
        femm.mi_selectrectangle((r_g-g*0.5)*np.cos(theta_p_r),(r_g-g*0.5)*np.sin(theta_p_r),r_outer*np.cos(np.deg2rad(-0.5)),r_outer*np.sin(np.deg2rad(-0.5)))

        femm.mi_copyrotate(x0,y0,theta_p_d*4,1)
        femm.mi_clearselected()
        seg=Segment(Point(r3[0].x,r3[0].y, evaluate=False),Point((rad+bridge_thickness)*np.cos(angle),(rad+bridge_thickness)*np.sin(angle),evaluate=False))
...
#       
        ...
        femm.mi_addnode(r_g*np.cos(np.deg2rad(theta_p_d*5)), r_g*np.sin(np.deg2rad(theta_p_d*5)))
        femm.mi_addarc(r_g*np.cos(0), r_g*np.sin(0), r_g*np.cos(np.deg2rad(theta_p_d*5)), r_g*np.sin(np.deg2rad(theta_p_d*5)),theta_p_d*5, 1)

        femm.mi_selectgroup(1)
#        
        
        femm.mi_addnode(r_outer*np.cos(0), r_outer*np.sin(0))
        femm.mi_addnode(r_outer*np.cos(np.deg2rad(theta_p_d*5)), r_outer*np.sin(np.deg2rad(theta_p_d*5)))
        femm.mi_addarc(r_outer*np.cos(0), r_outer*np.sin(0), r_outer*np.cos(theta_p_r*5), r_outer*np.sin(theta_p_r*5),theta_p_d*5, 1)
...
####       # Draw the stator slots and Stator coils 
        ...
        X_13,Y_13=(r_a-h_s1-h_s2-h_s/2)*np.cos(theta_tau_s_new2),(r_a-h_s1-h_s2-h_s/2)*np.sin(theta_tau_s_new2)
        X_14,Y_14=(r_a-h_s1-h_s2-h_s)*np.cos(theta_tau_s_new2),(r_a-h_s1-h_s2-h_s)*np.sin(theta_tau_s_new2)
        ...
        X_18,Y_18=(r_a-h_s1-h_s2-h_s/2)*np.cos(theta_tau_s_new4),(r_a-h_s1-h_s2-h_s/2)*np.sin(theta_tau_s_new4)
        X_19,Y_19=(r_a-h_s1-h_s2-h_s)*np.cos(theta_tau_s_new4),(r_a-h_s1-h_s2-h_s)*np.sin(theta_tau_s_new4)
        


        X_21,Y_21=(r_a-h_s1-h_s2-h_s-h_ys),0
        X_22,Y_22=(r_a-h_s1-h_s2-h_s-h_ys)*np.cos(theta_p_r*5),(r_a-h_s1-h_s2-h_s-h_ys)*np.sin(theta_p_r*5)  
###        
###        
##      
...
#        
        ...
        femm.mi_addarc(X_21,Y_21,X_22,Y_22,np.rad2deg(theta_p_r*5),1)
...
#        
        ...
        femm.mi_addblocklabel((r_a-h_s1-h_s2-h_s-h_ys*0.5)*np.cos(theta_p_r*2.5),(r_a-h_s1-h_s2-h_s-h_ys*0.5)*np.sin(theta_p_r*2.5))
        femm.mi_selectlabel((r_a-h_s1-h_s2-h_s-h_ys*0.5)*np.cos(theta_p_r*2.5),(r_a-h_s1-h_s2-h_s-h_ys*0.5)*np.sin(theta_p_r*2.5))
        femm.mi_setblockprop("M-36 Steel")
        femm.mi_clearselected()
        
        femm.mi_addblocklabel((r_outer-0.5*h_yr)*np.cos(theta_p_r*2.5),(r_outer-0.5*h_yr)*np.sin(theta_p_r*2.5))
        femm.mi_selectlabel((r_outer-0.5*h_yr)*np.cos(theta_p_r*2.5),(r_outer-0.5*h_yr)*np.sin(theta_p_r*2.5))
        femm.mi_setblockprop("M-36 Steel")
        femm.mi_clearselected()
#
        
        femm.mi_addblocklabel((D_a/2+(r_g-D_a/2)*0.5)*np.cos(theta_p_r*2.5),(D_a/2+(r_g-D_a/2)*0.5)*np.sin(theta_p_r*2.5))
        femm.mi_selectlabel((D_a/2+(r_g-D_a/2)*0.5)*np.cos(theta_p_r*2.5),(D_a/2+(r_g-D_a/2)*0.5)*np.sin(theta_p_r*2.5))
...
#         
        ...
        try:
            ...
            femm.smartmesh(0)
            femm.mi_analyze()
            outputs["B_g"], outputs["B_rymax"], outputs["B_smax"], outputs["Sigma_normal"],V_rotor,V_stator = run_post_process(D_a, g, r_outer,h_yr,h_ys, r_inner,theta_p_r)
            ...
            outputs["T_e"],outputs["Sigma_shear"] =B_r_B_t(Theta_elec,D_a, l_s, p1,g, theta_p_r, I_s, theta_tau_s,layer_1, layer_2, N_c, tau_p)
            if outputs["T_e"]>=20e6:
                seed=random.randrange(0, 101, 2)
                femm.mi_saveas("IPM_new_"+str(seed)+".fem")
           
        except:
            print (D_a,l_s,h_t,p1,g,h_ys,h_yr,alpha_v,N_c,I_s,h_m,d_mag,d_sep,m_sep*line2.length,l_fe_ratio,ratio)
