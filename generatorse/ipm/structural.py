...
# Unpack inputs
        ...
        u_d             =f/(2*constants_x_L[0]*(constants_x_L[1])**3)*((constants_x_L[2]/(2*constants_x_L[3])*constants_x_L[4] -constants_x_L[5]/constants_x_L[3]*constants_x_L[6]-0.5*constants_x_L[7]))+inputs['y_sh']
        ...
        a_i             =  R
        y_ai1r          = -W_ir*(R)**4/(R_sh*W_back_iron[0])*(W_back_iron[1]*W_back_iron[4]/W_back_iron[3]-W_back_iron[2])
        W_sr            =  rho_Fes*g1*np.sin((np.deg2rad(phi)))*(L_r-t_r)*h_yr
        y_ai2r          = -W_sr*(r_g+h_yr)**4/(R_sh*W_ssteel[0])*(W_ssteel[1]*W_ssteel[4]/W_ssteel[3]-W_ssteel[2])
        ...
        a_ii            = r_g
        r_oii           = R_sh
        M_rb            = -w_disc_r*a_ii**2/W_ssteel[3]*(W_ssteel[4]*0.5/(a_ii*R_sh)*(a_ii**2-r_oii**2)-W_ssteel[9])
        ...
        outputs['y_ar'] =(y_ai1r+y_ai2r+y_ai3r)+y_aiir+(R*inputs['theta_sh'])+M_ar*L_r**2*0/(2*E*I)
        ...
        outputs['Structural_mass_rotor'] = rho_Fes*np.pi*(((r_g)**2-(R_sh)**2)*t_r+\
                                           ((r_outer_active+h_sr)**2-(r_outer_active)**2)*L_r)
                                           
        outputs['con_uar'] = 1e3*(outputs['u_allowable_r'] - outputs['u_ar'])
        outputs['con_yar'] = 1e3*(outputs['y_allowable_r'] - outputs['y_ar'])
...
class PMSG_stator_inactive(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['u_as'] = (Sigma_normal*(R_is)**2)/(E*(h_ys+h_ss))-f/(2*constants_x_L[0]*(constants_x_L[1])**3)*((constants_x_L[2]/(2*constants_x_L[3])*constants_x_L[4] -constants_x_L[5]/constants_x_L[3]*constants_x_L[6]-1/2*constants_x_L[7]))+inputs['y_bd']
        ...
        y_ai1s           = -W_is*(h_ys*0.5+h_ss+R_is)**4/(R_no*W_back_iron[0])*(W_back_iron[1]*W_back_iron[4]/W_back_iron[3]-W_back_iron[2])
                
        W_ss            =  rho_Fes*g1*np.sin(np.deg2rad(phi))*(L_s-t_s)*h_ss
        y_ai2s           = -W_ss*(h_ss*0.5+R_is)**4/(R_no*W_ssteel[0])*(W_ssteel[1]*W_ssteel[4]/W_ssteel[3]-W_ssteel[2])
        W_cu            =  np.sin((np.deg2rad(phi)))*(M_Fest+Copper)/(2*np.pi*(inputs["D_a"]*0.5-h_t*0.5))
        y_ai3s           = -W_cu*(D_a*0.5-h_t*0.5)**4/(R_no*W_active[0])*(W_active[1]*W_active[4]/W_active[3]-W_active[2])
        
        w_disc_s        = rho_Fes*g1*np.sin((np.deg2rad(phi)))*t_s
        
        a_ii            = R_is
        r_oii           = R_no
        M_rb            = -w_disc_s*a_ii**2/W_ssteel[3]*(W_ssteel[4]*0.5/(a_ii*R_no)*(a_ii**2-r_oii**2)-W_ssteel[9])
        ...
        outputs['y_as'] =abs(y_ai1s+y_ai2s+y_ai3s+y_aiis+(D_a*0.5*inputs['theta_bd'])+M_as*L_s**2*0/(2*E*I))
        ...
        outputs['Structural_mass_stator'] = rho_Fes*(np.pi*((R_is**2-(R_no)**2)*t_s)+\
                                            np.pi*((R_is+h_ss)**2-R_is**2)*L_s)
            
        outputs["Structural_mass"] = outputs["Structural_mass_stator"] + Structural_mass_rotor
        
        outputs['con_uas'] = 1e3*(outputs['u_allowable_s'] - outputs['u_as'])
        outputs['con_yas'] = 1e3*(outputs['y_allowable_s'] - outputs['y_as'])
...
if __name__ == "__main__":
    ...
    prob.model.add_constraint('con_uar', lower=0.01)
    prob.model.add_constraint('con_yar', lower=0.01)
    prob.model.add_constraint("con_uas", lower=0.01)
    prob.model.add_constraint("con_yas", lower=0.01)
