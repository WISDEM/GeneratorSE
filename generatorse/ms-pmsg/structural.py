...
class PMSG_rotor_inactive(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs, discrete_outputs):
        ...
        w_r					=rho_Fes*g1*np.sin((np.deg2rad(phi)))*a_r																	 # uniformly distributed load of the weight of the rotor arm
        outputs['mass_Fe']=mass_st_lam=rho_Fe*2*np.pi*(R)*l*h_yr                                     # mass of rotor yoke steel
        W				=g1*np.sin((np.deg2rad(phi)))*(mass_st_lam/N_r+(outputs['mass_PM'])/N_r) 											 # weight of 1/nth of rotor cylinder
        ...
        outputs['z_A_r']      =(2*np.pi*(R-0.5*t)*l/N_r)*Sigma_shear*(l_ir-0.5*t)**3/3/E/I_arm_tor_r       # circumferential deflection
        outputs["Structural_rotor"]  =(N_r*(R_1-R_sh)*a_r*rho_Fes)+np.pi*((r_g-g-h_m-h_yr)**2-(r_g-g-h_m-t)**2)*l*rho_Fes
...
class PMSG_stator_inactive(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs, discrete_outputs):
        ...
        l=L_t=inputs["L_t"]   # end winding length
        phi=inputs['phi']
        ...
        mass_st_lam_s= M_Fest+np.pi*L_t*rho_Fe*((r_g+h_s+h_s1+h_s2+h_ys)**2-(r_g+h_s+h_s1+h_s2)**2)
        W_is			=0.5*g1*np.sin(np.deg2rad(phi))*(rho_Fes*l*d_s**2)                          # length of stator arm beam at which self-weight acts
        W_iis     =g1*np.sin((np.deg2rad(phi)))*(mass_st_lam_s+Copper+np.pi*((R_st +t_s*0.5)**2-(R_st +t_s*0.5-h_ss)**2)*l*rho_Fes)/N_st	 # weight of stator cylinder and teeth
        w_s         =rho_Fes*g1*np.sin(np.deg2rad(phi))*a_s									 # uniformly distributed load of the arms
            		
        #print (M_Fest+self.Copper)*g1
            		
        mass_stru_steel  =2*(N_st*(R_1s-R_no)*a_s*rho_Fes)+	np.pi*((R_st +t_s*0.5)**2-(R_st +t_s*0.5-h_ss)**2)*l*rho_Fes

       
        outputs["y_all"]=y_all_pcent*l/100
        # Calculating radial deflection of the stator
            		
        Numers=R_st**3*((0.25*(np.sin(theta_s)-(theta_s*np.cos(theta_s)))/(np.sin(theta_s))**2)-(0.5/np.sin(theta_s))+(0.5/theta_s))
        ...
        outputs["z_A_s"]  =np.pi*(R_st+0.5*t_s)*l*2/(2*N_st)*Sigma_shear*(l_is+0.5*t_s)**3/3/E/I_arm_tor_s 
