...
#        femm.mi_clearselected()
        ...
        femm.mi_addblocklabel((r_m-h_m-h_yr*0.5)*np.cos(theta_p_r*0.5),(r_m-h_m-h_yr*0.5)*np.sin(theta_p_r*0.5))
        femm.mi_selectlabel((r_m-h_m-h_yr*0.5)*np.cos(theta_p_r*0.5),(r_m-h_m-h_yr*0.5)*np.sin(theta_p_r*0.5))
...
####     
        ...
        X_13,Y_13=(r_g+h_s1+h_s2+h_s/2)*np.cos(theta_tau_s_new2),(r_g+h_s1+h_s2+h_s/2)*np.sin(theta_tau_s_new2)
        X_14,Y_14=(r_g+h_s1+h_s2+h_s)*np.cos(theta_tau_s_new2),(r_g+h_s1+h_s2+h_s)*np.sin(theta_tau_s_new2)
        ...
        X_18,Y_18=(r_g+h_s1+h_s2+h_s/2)*np.cos(theta_tau_s_new4),(r_g+h_s1+h_s2+h_s/2)*np.sin(theta_tau_s_new4)
        X_19,Y_19=(r_g+h_s1+h_s2+h_s)*np.cos(theta_tau_s_new4),(r_g+h_s1+h_s2+h_s)*np.sin(theta_tau_s_new4)
        

        X_20,Y_20=r_m,0
        X_21,Y_21=(r_g+h_s1+h_s2+h_s+h_ys),0
        X_22,Y_22=(r_g+h_s1+h_s2+h_s+h_ys)*np.cos(theta_p_r*1),(r_g+h_s1+h_s2+h_s+h_ys)*np.sin(theta_p_r*1)  
###        
###        
##      
...
##      
        ...
        femm.mi_addblocklabel((r_g+h_s1+h_s2+h_s+h_ys*0.5)*np.cos(theta_p_r*0.5),(r_g+h_s1+h_s2+h_s+h_ys*0.5)*np.sin(theta_p_r*0.5))
        femm.mi_selectlabel((r_g+h_s1+h_s2+h_s+h_ys*0.5)*np.cos(theta_p_r*0.5),(r_g+h_s1+h_s2+h_s+h_ys*0.5)*np.sin(theta_p_r*0.5))
        femm.mi_setblockprop("M-36 Steel")
        femm.mi_clearselected()
...
##         
        ...
        try:
            ...
            femm.smartmesh(0)
            femm.mi_analyze()
            outputs["B_g"], outputs["B_rymax"], outputs["B_symax"], outputs["B_tmax"], outputs["Sigma_normal"],V_rotor,V_stator = run_post_process(r_g, g, r_outer,h_yr,h_ys, r_m, yoke_radius,theta_p_r)
            ...
            outputs["T_e"],outputs["Sigma_shear"] =B_r_B_t(Theta_elec,r_g, l_s, p,g, theta_p_r, I_s, theta_tau_s,layer_1, layer_2, N_c)
            if outputs["T_e"]>=20e6:
