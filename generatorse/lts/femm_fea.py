...
class FEMM_Geometry(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs, discrete_outputs):
        ...
        femm.mi_addcircprop("A1+", I_sc_in, 1)
        femm.mi_addcircprop("A1-", -1 * I_sc_in, 1)
        ...
        try:
            ...
            I_sc = I_sc_in
            bisec_err = 1e3
            while bisec_err > 1 and n < 3:
                femm.mi_analyze()
                femm.mi_loadsolution()
                B_g, B_rymax, B_coil_max, sigma_n = run_post_process(D_a, radius_sc, h_sc, slot_radius, theta_p_r, alpha_r, beta_r, n)
                ...
                femm.mi_modifycircprop("A1-",  1, -1 * I_sc)
                femm.mi_saveas("coil_design_new.fem")

            outputs["margin_I_c"] = outputs["I_sc_out"] = I_sc
            outputs["B_g"], outputs["B_rymax"], outputs["B_coil_max"], outputs["Sigma_normal"] = B_g, B_rymax, B_coil_max, sigma_n
            outputs["constr_B_g_coil"] = B_g / B_coil_max
