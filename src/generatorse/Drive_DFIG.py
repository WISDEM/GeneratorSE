...
class Drive_DFIG(component):
 ...
 S_N =Float(iotype='in', desc='Stator slot height ')
 B_symax = Float(iotype='in', desc='Peak Yoke flux density B_ymax')
 I_f= Float(iotype='in', desc='Rotor current at no-load')
 ...
 def execute(self):
  ...
  P_total_loss=TL #+ P_conv;
	...
	def configure(self):
		...
		# Iteration Hierarchy
		self.driver.workflow.add('Drive_DFIG')
		# Design Variable
		self.driver.add_parameter('Drive_DFIG.r_s', low=0.2, high=1)
    self.driver.add_parameter('Drive_DFIG.l_s', low=0.4, high=2)
    self.driver.add_parameter('Drive_DFIG.h_s', low=0.045, high=0.1)
    self.driver.add_parameter('Drive_DFIG.h_r', low=0.045, high=0.1)
    #self.driver.add_parameter('DFIG_Optim5MW.h_ys', low=0.04, high=0.15)
    self.driver.add_parameter('Drive_DFIG.B_symax', low=1, high=2)
    self.driver.add_parameter('Drive_DFIG.S_N', low=-0.3, high=-0.002)
    self.driver.add_parameter('Drive_DFIG.I_f', low=5, high=100)
    if (B==1):
    ...
    if (C==1) or (C==2) or (C==3) or (C==5):
		...
		Optim.DFIG_r_s= 0.5  #meter
    Optim.DFIG_l_s= 1.5 #meter
    Optim.DFIG_h_s = 0.1 #meter
    Optim.DFIG_h_r = 0.1 #meter
    Optim.DFIG_I_0 = 14.5  # Ampere
    Optim.DFIG_B_symax = 1. # Tesla
    Optim.DFIG_S_N = -0.1 # Tesla