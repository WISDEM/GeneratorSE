...
class PMDDOptimdiscrotor_exec(Component):
 ...
 h_yr= Float(0.0, iotype='in', desc='thickness of arms t')
 h_ys= Float(0.0, iotype='in', desc='thickness of arms t')
 ...
 h_m =Float(0.025, iotype='in', desc='Magnet height')
 t_d =Float(0.0, iotype='in', desc='disc thickness self.t_d')
 n_s =Float(0, iotype='in', desc='number of arms n')
 t_s= Float(0.0, iotype='out', desc='stator back iron t')
 ...
 W_1a =Float(0, iotype='out', desc='Number of turns in the stator winding')
 ...
 h_m=Float(0.0, iotype='out', desc='magnet height')
 ...
 Cost_actual= Float(0.0, iotype='out', desc='Total cost')
 ...
 Iron=Float(0.0, iotype='out', desc='Active mass')  
 PM=Float(0.0, iotype='out', desc='Stator mass')
 Copper=Float(0.0, iotype='out', desc='rotor mass')
 Inactive=Float(0.0, iotype='out', desc='rotor mass')
 A=Float(0.0, iotype='out', desc='rotor mass')
 Axial_delta_stator=Float(0.0, iotype='out', desc='rotor mass')
 Radial_delta_stator=Float(0.0, iotype='out', desc='rotor mass')
 Circum_delta_stator=Float(0.0, iotype='out', desc='rotor mass')
 Axial_delta_rotor=Float(0.0, iotype='out', desc='rotor mass')
 Radial_delta_rotor=Float(0.0, iotype='out', desc='rotor mass')
 P_gennom=Float(0.0, iotype='out', desc='Power')
 ...
 N_slots=Float(0.1, iotype='out', desc='Number of slots')
 ...
 K_load			=Float(0.1, iotype='out', desc='Load ratio')
 ...
 def execute(self):
  ...
  rho_Cu=1.8*10**(-8)*1.4
  k_fes =0.9
  R_o = 0.43 # #0.523950817 #0.43
  k_sfil =0.65								 # Slot fill factor
  T = 4143289.8# # 1790493.11 #7.29e6 #250418.6168  #1790493.11 #4143289.8 #7.29e6 #250418.6168  #698729.0185698729.0185 #1790493.11 # 10MW 9549296.586   #4143289.8 5MW
  self.P_gennom = 5e6 #0.75e6 #10e6    #5.25e6
  P_gridnom=3e6
  E_pnom   = 3000
  n_nom = 12.1 #12.1 # 7.2 #28.6 #20.5 #16 #10 10MW    #12.1 5MW
  ...
  #h_ys =self.B_g*self.b_m*l_e/(2*self.B_ymax*l_u) 
  #h_ys =self.h_yr
  ...
  #self.h_m  =c/B_r/K/((1/self.B_g)-(1/(B_r*K)))  
  ...
  #self.h_yr =0.5*B_r/B_rymax*self.tau_p*ratio*(h_m/mu_r)/((h_m/mu_r)+c)  # rotor yoke height
  v = 0.3
  r_r				=  self.r_s-g              #rotor radius
  self.p		=  round(pi*dia/(2*self.tau_p))   # number of pole pairs
  N_slot		=  2
  self.N_slots=N_slot*self.p*q1*m        #number of turns of stator phase winding
  ...
  #y_ai=-(W*R_b**3/(R_o*D_ax))*(C_2p*C_6/C_5-C_3p)
  ...
  self.z_all     =0.05*2*pi*R_st/360  # allowable torsional deflection
  ...
  self.Radial_delta_rotor=abs(self.u_Ar)
  ...
  L_ssigmas	= 2*mu_0*L_t*(self.W_1a)**2/self.p/q1*((self.h_s-0.004)/(3*self.b_s)+0.004/b_so)  #slot leakage inductance
  ...
  I_snom		=self.P_gennom/m/self.E_p/cofi 
  ...
  W_iis     =g1*sin(phi)*(mass_st_lam_s+M_Cus)/2/N_st
  ...
  self.TC=95*mass_PM+0.556*((pi*l*(R_a**2-R**2)*rho))+0.50139*(pi*self.t_d*(R**2-R_o**2)*rho)+0.50139*mass_stru_steel+0.556*mass_st_lam_s+M_Cus*K_Cu
 ...
 #"% losses %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
  ...
  #self.A= N_s*q1*m*self.I_s/self.tau_p
  ...
  #"% cost of losses"
  self.Cost=K_gen+0.50139*mass_stru_steel+0.556*mass_st_lam_s+95*mass_PM+0.50139*((pi*l*(R_a**2-R**2)*rho)+pi*self.t_d*(R**2-R_o**2)*rho)
  ...
  R_1=1/(3*self.l_s*self.b_t*alpha_1)
  ...
  R_24=0.5*h_m/(self.l_s*self.b_m*L_mag)
  R_25=-R_24/3
  ...
  #print M_actual
  #print delta_t
  #print self.W_1a,self.r_s,k_wd,k_wskew,B_pm1
  
  self.Iron=mass_st_lam_s+(pi*l*(R_a**2-R**2)*7700)
  self.PM=mass_PM
  self.Copper=M_Cus
  self.Inactive=mass_stru_steel+(pi*(R**2-R_o**2)*self.t_d*rho)
  self.M_actual=self.Iron+self.PM+self.Copper+self.Inactive
  
  print mass_PM*g1,(M_Fest+M_Cus)*g1
