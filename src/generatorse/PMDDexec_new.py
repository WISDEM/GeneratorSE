...
class PMDDexec_new(Component):
 ...
 def execute(self):
  ...
  #alpha_p1=0.;
  #c_p=2*alpha_p1/self.b_m;
  #b_t1=(self.b_m-self.tau_p)/2;
  #gamma=(4/pi)*(b_so/2/g*atan(b_so/2/g)-log(sqrt(1+(b_so**2/4/g**2)))); 
  #Order=input('Enter the order of slot harmonics to be included :')
  #k_su= np.zeros(Order);
  #b_u=np.zeros(Order);
  #mu1=np.zeros(Order);
  #b_fs=1
  ...
  #band = np.arange(0, Order, 1)
  #for h in band:
  # mu1[h]=6*(h+1)+1
  # k_su[h]=sin(mu1[h]*b_fs*pi/self.tau_p)/(mu1[h]*b_fs*pi/self.tau_p);
  # b_u[h]=4/self.tau_p*((c_p/(c_p**2+(mu1[h]*pi/self.tau_p)**2)*sinh(alpha_p)-6*(self.tau_p/mu1[h]/pi)**4*1/b_t1**3*cosh(alpha_p1))+3*(self.tau_p/mu1[h]/pi)**2*(2/(self.tau_p-self.b_m)*cosh(alpha_p1)))*sin(mu1[h]*pi*0.5)*sin(mu1[h]*pi*b_t1/self.tau_p)+4/self.tau_p*(mu1[h]*pi/self.tau_p/(c_p**2+(mu1[h]*pi/self.tau_p)**2)+6*(self.tau_p/mu1[h]/pi)**3*(1/b_t1**2)-self.tau_p/mu1[h]/pi)*cosh(alpha_p1)*sin(mu1[h]*pi/2)*cosh(mu1[h]*pi*b_t1/self.tau_p);
   # c=c+1
   #h=h+1 
  #Angle = np.arange(0, 2*pi, 0.02)
  #total=len(Angle);
  #B_pm=np.zeros(total);
  #Q=np.zeros(total);
  #B_sl=np.zeros(total);
  #B_final=np.zeros(total);
  #band3=  np.arange(0, total,1)
    
  #rho1=b_so/tau_s *2*(1+(b_so/tau_s)**2)**0.5/((5+b_so/tau_s)*(1+(b_so/tau_s)**2)**0.5-1)
  #getx=lambda n: self.B_g*b_u[n]*k_su[n]*cos(mu1[n]*pi*Angle[k]/self.tau_p)
  #gety=lambda r: sin(r*pi*rho1*b_so/(2*tau_s))/(r*pi*rho1*b_so/(2*tau_s))*sin(r*pi*rho1*b_so/(2*tau_s))/(r*pi*rho1*b_so/(2*tau_s))*sin(r*b_sk*pi/tau_s)/(r*b_sk*pi/tau_s)*sin(r*b_sk*pi/tau_s)/(r*b_sk*pi/tau_s)*cos(r*2*pi*Angle[k]/tau_s)*B_pm[k]
  #for k in band3:
	#B_pm[k]=sum(getx(n) for n in band)
	#Q[k]= nsum(gety,[1,inf])
	# B_sl[k]= -2*gamma*g/tau_s*Q[k] 
  ##B_pm =B_pm/k_C
  #B_final=B_pm+B_sl
  ...
  #writer = pd.ExcelWriter('Flux_density_plot.xlsx')
  #table.to_excel(writer, 'Flux')
  #writer.save()
  #print total,len(B_final)
  #import matplotlib.pyplot as plt
  #plt.plot(Angle,B_final,'r-')
  #plt.ylabel('Air-gap Flux Density (Tesla)')
  #plt.xlabel('Location along circumference (rad)')
  #plt.show()
