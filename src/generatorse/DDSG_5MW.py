...
class DDSG_5MW(Component):
 """ Evaluates the total cost """
 r_s = Float(3.2, iotype='in', desc='airgap radius r_s')
 l_s = Float(1.4, iotype='in', desc='Stator core length l_s')
 h_s = Float(0.060, iotype='in', desc='Yoke height h_s')
 tau_p =Float(0.17, iotype='in', desc='Pole pitch self.tau_p')
 B_g = Float(0, iotype='out', desc='Peak air gap flux density B_g')
 B_gfm = Float(0, iotype='out', desc='no load air gap flux density B_g')
 B_symax = Float(0, iotype='out', desc='Peak Stator Yoke flux density self.B_symax')
 B_rymax = Float(0, iotype='out', desc='Peak rotor Yoke flux density B_rymax')
 B_tmax= Float(0, iotype='out', desc='Peak tooth density self.B_tmax')
 B_gfm=Float(0, iotype='out', desc='Peak tooth density self.B_tmax')
 B_pc=Float(0, iotype='out', desc='Pole core flux density')
 N_f=Float(100, iotype='in', desc='Peak Yoke flux density B_ymax')
 I_f=Float(69, iotype='in', desc='Generator field current')
 W_1a=Float(0, iotype='out', desc='Peak Yoke flux density B_ymax')
 b_s=Float(0.1, iotype='out', desc='slot width')
 h_ys=Float(0.13, iotype='in', desc='Yoke height') #cost 0.1805
 h_yr=Float(0.08, iotype='in', desc='rotor yoke height') #cost 0.1805
 ...
 p=Float(60, iotype='out', desc='No of pole pairs')
 f=Float(0.0, iotype='out', desc='Output frequency')
 E_s=Float(1000, iotype='out', desc='Stator phase voltage')
 I_s=Float(0.0, iotype='out', desc='Generator output phase current')
 A_Cuscalc=Float(0.0, iotype='out', desc='Conductor cross-section')
 A_Curcalc=Float(0.0, iotype='out', desc='rotor Conductor cross-section')
 A_1=Float(0.0, iotype='out', desc='Specific loading')
 ...
 Iron=Float(0.0, iotype='out', desc='Generator efficiency')
 Copper=Float(0.0, iotype='out', desc='Generator efficiency')
 Inactive=Float(0.0, iotype='out', desc='Generator efficiency')
 #Structural variables
 n_s =Float(5, iotype='in', desc='number of arms n')
 b_st = Float(0.54,iotype='in', desc='arm width b_r') #0.576
 d_s = Float(0.52,iotype='in', desc='arm depth d_r') #0.607 mass and cost
 t_ws =Float(0.07, iotype='in', desc='arm depth thickness self.t_wr')
 #rotor
 n_r =Float(5, iotype='in', desc='number of arms n')
 b_r = Float(0.53, iotype='in', desc='arm width b') # cost 0.61
 d_r = Float(0.55, iotype='in', desc='arm depth d') #0.6 mass and cost
 t_wr =Float(0.2, iotype='in', desc='arm depth thickness self.t_w') #0.11
 J_f =Float(0.1, iotype='out', desc='Rotor Current density')
 Cost=Float(0.1, iotype='out', desc='Rotor Current density')
 ...
 n_brushes=Float(0.1, iotype='out', desc='brushes') 
 P_gennom=Float(0.1, iotype='out', desc='Power rating') 
 Load_mmf_ratio=Float(0.1, iotype='out', desc='mmf_ratio') 
 Power_ratio=Float(0.1, iotype='out', desc='Power_ratio') 
 Slot_aspect_ratio=Float(0.1, iotype='out', desc='Slot_aspect_ratio') 
 ...
 def execute(self):
  ...
  k_fes =0.9
  R_o = 0.43
  T = 4143289.841 #250418.6168 #698729.0185 #1790493.11
  self.P_gennom = 5e6
  ...
  #self.h_ys= self.B_g*self.tau_p/(self.B_ymax*pi)
  #self.h_yr =self.h_ys
  ...
  self.B_g=self.B_gfm*4*sin(0.5*b_p*pi/self.tau_p)/pi  # fundamental component
  
  
  tau_r=0.5*(b_p-2*k_s*self.tau_p/(6*q1+1)) #damper bar pitch
  k_fillr = 0.7
  ...
  self.B_tmax=0.5*(self.B_gfm+self.B_g)*tau_s/b_t  #Stator teeth flux density
  self.B_symax=self.tau_p*self.B_g/pi/self.h_ys #stator yoke flux density
  L_fg=2*mu_0*self.p*self.l_s*4*self.N_f**2*((h_ps/(self.tau_p-b_p))+(h_pc/(3*pi*(r_r-h_pc-h_ps)/self.p-b_pc)))
  ...
  delta_m=0
  self.B_pc=(1/b_pc)*((2*self.tau_p/pi)*self.B_g*cos(delta_m)+(2*mu_0*self.I_f*self.N_f*((2*h_ps/(self.tau_p-b_p))+(h_pc/(self.tau_p-b_pc)))))
  ...
  L_dm= (b_p/self.tau_p +(1/pi)*sin(pi*b_p/self.tau_p))*self.L_m
  L_qm=(b_p/self.tau_p -(1/pi)*sin(pi*b_p/self.tau_p)+2/(3*pi)*cos(b_p*pi/2*self.tau_p))*self.L_m
  ...
  I_f2=(E_p/self.E_s)*self.B_g*g_eff*pi/(4*self.N_f*mu_0*sin(pi*b_p/2/self.tau_p))
  phi_max_stator=k_wd*self.W_1a*pi*self.r_s*self.l_s*2*mu_0*self.N_f*self.I_f*4*sin(0.5*b_p/self.tau_p/pi)/(self.p*pi*g_eff*pi)
  #M_sf=mu_0*8*self.r_s*self.l_s*k_wd*self.W_1a*self.N_f*sin(0.5*b_p/self.tau_p/pi)/(self.p*g_eff*pi)
  ...
  V_Fert=2*self.p*l_pfe*(h_pc*b_pc+b_p*h_ps)
  ...
  print ((V_Cusr*8900)+M_Fert)*g1,(V_Cuss*8900+M_Fest)*g1,W,l_ir,E,I_arm_axi_r,w_r,l_iir
