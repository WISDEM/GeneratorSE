...
class PMSG(Component):
 ...
 #Stator design
 W_1a =Float(iotype='out', desc='Number of turns in the stator winding')
 ...
 def execute(self):
  ...
  self.Rotor_delta_radial=u_Ar
  self.Rotor_delta_axial=y_Ar
 ...
 #"% losses %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
  ...
  print ((self.mass_PM))*g1,  (self.M_Fest+V_Cus*8900)*g1
...
class Drive_PMSG(Assembly):
	...
	Generator_Torque=Float(iotype='in', desc='Torque')
	Generator_speed=Float(iotype='in', desc='rated speed')
	Generator_input=Float(iotype='in', desc='rated power')
	...
	def __init__(self,Optimiser='',Objective_function=''):
		...
		self.connect('PMSG_t_s','PMSG.t_s')
		self.connect('PMSG_t','PMSG.t')
		...
		self.connect('Generator_input','PMSG.machine_rating')
		self.connect('Generator_speed','PMSG.n_nom')
		self.connect('main_shaft_cm','PMSG.main_shaft_cm')
		self.connect('main_shaft_length','PMSG.main_shaft_length')
		self.connect('Generator_Torque','PMSG.Torque')
		...
		self.driver.design_vars=['PMSG_r_s','PMSG_l_s','PMSG_h_s','PMSG_tau_p','PMSG_h_m','PMSG_n_r','PMSG_n_s','PMSG_b_r','PMSG_b_st','PMSG_d_r','PMSG_d_st','PMSG_t_wr','PMSG_t_ws']
		...
		self.driver.add_constraint('PMSG.gen_eff>=Target_Efficiency')			#26
...
#	# Initial design variables for a DD PMSG designed for a 5MW turbine
	opt_problem.Generator_input=5.0e6
	opt_problem.Generator_Torque=4.143289e6
	opt_problem.Generator_speed=12.1
...
#	opt_problem.Generator_input=0.75e6
#	opt_problem.Generator_Torque=250418.6168
#	opt_problem.Generator_speed=28.6
#	opt_problem.PMSG_r_s=1.3              
#	opt_problem.PMSG_l_s= 0.7
#	opt_problem.PMSG_h_s = 0.045
#	opt_problem.PMSG_tau_p = 0.045
#	opt_problem.PMSG_h_m = 0.009
#	opt_problem.PMSG_t_s = 0.05
#	opt_problem.PMSG_t = 0.05
#	opt_problem.PMSG_n_s = 5
#	opt_problem.PMSG_b_st = 0.22
#	opt_problem.PMSG_n_r =5
#	opt_problem.PMSG_b_r = 0.22
#	opt_problem.PMSG_d_r = 0.33
#	opt_problem.PMSG_d_s= 0.17
#	opt_problem.PMSG_t_wr =0.01
#	opt_problem.PMSG_t_ws =0.01
#	opt_problem.PMSG_R_o =0.17625 
	opt_problem.run()
	...
	print df
	df.to_excel('PMDD_'+str(opt_problem.Generator_input/1e6)+'MW.xlsx')
